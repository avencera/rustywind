...
for failure_type, count in sorted_failures[:20]:
    print(f"{count:3d}x {failure_type}")

# Save detailed results
with open('fuzz_25run_detailed.json', 'w') as f:
    json.dump({
        'results': results,
...
print(f"\nDetailed results saved to: fuzz_25run_detailed.json")
