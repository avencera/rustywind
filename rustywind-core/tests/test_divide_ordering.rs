...
fn test_divide_reverse_vs_other_divide_utilities() {
    ...
    // Tailwind orders divide-y-reverse before divide styles, and divide-x-reverse last.
    assert!(
        divide_y_reverse_pos < divide_solid_pos,
        "divide-y-reverse should come before divide-solid"
    );
    assert!(
        divide_y_reverse_pos < divide_dashed_pos,
        "divide-y-reverse should come before divide-dashed"
    );
    assert!(
        divide_y_reverse_pos < divide_dotted_pos,
        "divide-y-reverse should come before divide-dotted"
    );
    assert!(
        divide_y_reverse_pos < divide_double_pos,
        "divide-y-reverse should come before divide-double"
    );
    assert!(
        divide_y_reverse_pos < divide_none_pos,
        "divide-y-reverse should come before divide-none"
    );
    ...
    assert!(
        divide_y_reverse_pos < divide_x_reverse_pos,
        "divide-y-reverse should come before divide-x-reverse"
    );
}
...
fn test_divide_reverse_mixed_comprehensive() {
    ...
    let divide_y_4_pos = sorted.iter().position(|&c| c == "divide-y-4").unwrap();

    // Expected order (following Tailwind's property order):
    // 1. Divide width utilities (divide-x-2, divide-y-4)
    // 2. Divide-y-reverse
    // 3. Divide style utilities (divide-solid, divide-dashed)
    // 4. Positioning utilities (self-start)
    // 5. Overflow utilities (overflow-hidden)
    // 6. Border utilities (border-2, border-gray-300)
    // 7. Divide-x-reverse (falls back to its custom property, so it ends up last)

    assert!(
        divide_x_2_pos < divide_y_reverse_pos && divide_y_4_pos < divide_y_reverse_pos,
        "divide width utilities should come before divide-y-reverse"
    );
    assert!(
        divide_y_reverse_pos < divide_solid_pos && divide_y_reverse_pos < divide_dashed_pos,
        "divide-y-reverse should come before divide styles"
    );
    assert!(
        divide_solid_pos < self_start_pos && divide_dashed_pos < self_start_pos,
        "divide styles should come before positioning utilities"
    );
    assert!(
    ...
    assert!(
        overflow_pos < border_2_pos && overflow_pos < border_color_pos,
        "overflow should come before border"
    );
    assert!(
        border_2_pos < divide_x_reverse_pos && border_color_pos < divide_x_reverse_pos,
        "border utilities should come before divide-x-reverse"
    );
}
...
fn test_padding_vs_divide_reverse() {
    // Tailwind currently places divide-y-reverse before padding utilities and divide-x-reverse after them
    ...
    // Tailwind orders divide-y-reverse before padding utilities, and padding before divide-x-reverse.
    assert!(
        divide_y_reverse_pos < px_2_pos
            && divide_y_reverse_pos < pr_4_pos
            && divide_y_reverse_pos < pb_4_pos
            && divide_y_reverse_pos < pl_2_pos
            && divide_y_reverse_pos < p_4_pos,
        "divide-y-reverse should come before padding utilities"
    );
    assert!(
        ...
        "padding utilities should come before divide-x-reverse"
    );
    assert!(
        divide_y_reverse_pos < divide_x_reverse_pos,
        "divide-y-reverse should come before divide-x-reverse"
...
fn test_divide_reverse_specific_failures_from_100run() {
    ...
    let divide_solid_pos = sorted.iter().position(|&c| c == "divide-solid").unwrap();
    let self_center_pos = sorted.iter().position(|&c| c == "self-center").unwrap();
    let self_baseline_pos = sorted.iter().position(|&c| c == "self-baseline").unwrap();
    let rounded_t_pos = sorted.iter().position(|&c| c == "rounded-t").unwrap();
    let overflow_y_hidden_pos = sorted
        .iter()
        .position(|&c| c == "overflow-y-hidden")
        .unwrap();
    let overflow_visible_pos = sorted
        .iter()
        .position(|&c| c == "overflow-visible")
        .unwrap();
    let divide_white_pos = sorted.iter().position(|&c| c == "divide-white").unwrap();
    let divide_transparent_pos = sorted
        .iter()
    ...
    // All these should come AFTER divide-y-reverse according to Tailwind's property order
    assert!(divide_y_reverse_pos < divide_solid_pos);
    assert!(divide_y_reverse_pos < divide_white_pos);
    assert!(divide_y_reverse_pos < divide_transparent_pos);
    assert!(divide_y_reverse_pos < divide_none_pos);
    assert!(divide_y_reverse_pos < self_center_pos);
    assert!(divide_y_reverse_pos < self_baseline_pos);
    assert!(divide_y_reverse_pos < overflow_y_hidden_pos);
    assert!(divide_y_reverse_pos < overflow_visible_pos);
    assert!(divide_y_reverse_pos < rounded_t_pos);

    // Padding/background utilities should still precede divide-x-reverse
    assert!(bg_blue_pos < divide_x_reverse_pos);
    assert!(px_2_pos < divide_x_reverse_pos);
    assert!(pr_4_pos < divide_x_reverse_pos);
