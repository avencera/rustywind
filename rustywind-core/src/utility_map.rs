...
impl UtilityMap {
    ...
    pub fn get_properties(&self, utility: &str) -> Option<&'static [&'static str]> {
        self.get_properties_with_named_color_inference(utility, true)
    }

    /// Get the CSS properties generated by a utility class with configurable named-color inference
    ///
    /// When `infer_named_colors` is false, project-defined names such as `bg-muted` or
    /// `text-display` remain unknown while built-in, arbitrary, and CSS-variable colors remain
    /// recognized
    pub fn get_properties_with_named_color_inference(
        &self,
        utility: &str,
        ...
        self.match_pattern(utility, infer_named_colors)
    }
    ...
    fn match_pattern(
        ...
        if base == "mask"
            && let Some(props) = mask_properties(value, infer_named_colors)
        {
            return Some(props);
        }
        ...
        match base {
            ...
            "bg" if is_color_like_value(value, infer_named_colors) => {
            ...
            "border"
                if value.is_empty()
                    ...
                    || is_parenthesized_length(value)
                    || (value.starts_with('[')
                        && !is_color_like_value(value, infer_named_colors)) =>
            {
                Some(&["border-width"][..])
            }
            "border-x" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-inline-color"][..])
            }
            "border-y" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-block-color"][..])
            }
            "border-s" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-inline-start-color"][..])
            }
            "border-e" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-inline-end-color"][..])
            }
            "border-t" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-top-color"][..])
            }
            "border-r" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-right-color"][..])
            }
            "border-b" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-bottom-color"][..])
            }
            "border-l" if is_color_like_value(value, infer_named_colors) => {
            ...
            // border color
            "border" if is_color_like_value(value, infer_named_colors) => {
                Some(&["border-color"][..])
            }
            ...
            // size/`[` must win before the named-color denylist (e.g. text-sm, text-[14px])
            "text" if is_known_color_like_value(value) => Some(&["color"][..]),
            "text" if is_size_keyword(value) => Some(&["font-size"][..]),
            // arbitrary and typed-length text sizes: text-[14px], text-(length:--fs)
            "text" if value.starts_with('[') || is_parenthesized_length(value) => {
                Some(&["font-size"][..])
            }
            "text" if infer_named_colors && is_named_color_value(value) => Some(&["color"][..]),
            ...
            "inset-shadow" if is_known_color_like_value(value) => {
                Some(&["--tw-inset-shadow-color"][..])
            }
            "inset-shadow" => Some(&["--tw-inset-shadow"][..]),
            "text-shadow" if value.starts_with('[') => Some(&["--tw-text-shadow"][..]),
            "text-shadow" if is_known_color_like_value(value) => {
                Some(&["--tw-text-shadow-color"][..])
            }
            ...
            "ring" if is_ring_color_value(value, infer_named_colors) => {
                Some(&["--tw-ring-color"][..])
            }
            "ring-offset" if value.parse::<u32>().is_ok() || is_parenthesized_length(value) => {
                Some(&["--tw-ring-offset-width"][..])
            }
            "ring-offset" if is_ring_color_value(value, infer_named_colors) => {
            ...
            "inset-ring" if is_ring_color_value(value, infer_named_colors) => {
            ...
            "drop-shadow" if is_known_color_like_value(value) => {
                Some(&["--tw-drop-shadow-color"][..])
            }
            ...
            "mask-radial" => Some(mask_radial_properties(value, infer_named_colors)),
            ...
            "outline" if is_color_like_value(value, infer_named_colors) => {
                Some(&["outline-color"][..])
            }
            "outline-offset" => Some(&["outline-offset"][..]),

            // accent color
            "accent" if is_color_like_value(value, infer_named_colors) || value == "auto" => {
                Some(&["accent-color"][..])
            }

            // caret color
            "caret" if is_color_like_value(value, infer_named_colors) => Some(&["caret-color"][..]),

            // placeholder color
            "placeholder" if is_color_like_value(value, infer_named_colors) => {
                Some(&["placeholder-color"][..])
            }

            // svg paint
            "fill" if is_color_like_value(value, infer_named_colors) => Some(&["fill"][..]),
            "stroke" if value.parse::<u32>().is_ok() || is_arbitrary_like_value(value) => {
                Some(&["stroke-width"][..])
            }
            "stroke" if is_color_like_value(value, infer_named_colors) => Some(&["stroke"][..]),
            ...
            // divide
            "divide-x" if is_color_like_value(value, infer_named_colors) => {
                Some(&["--tw-divide-color-sort"][..])
            }
            "divide-y" if is_color_like_value(value, infer_named_colors) => {
                Some(&["--tw-divide-color-sort"][..])
            }
            ...
            "divide" if is_color_like_value(value, infer_named_colors) => {
                Some(&["divide-color"][..])
            }
            ...
            "from" if is_color_like_value(value, infer_named_colors) => {
                Some(&["--tw-gradient-from"][..])
            }
            "via" if is_color_like_value(value, infer_named_colors) => {
                Some(&["--tw-gradient-via"][..])
            }
            "to" if is_color_like_value(value, infer_named_colors) => {
                Some(&["--tw-gradient-to"][..])
            }
            ...
            // text decoration
            "decoration" if is_color_like_value(value, infer_named_colors) => {
                Some(&["text-decoration-color"][..])
            }
            "decoration" if value.parse::<u32>().is_ok() || is_parenthesized_length(value) => {
...
fn mask_properties(value: &str, infer_named_colors: bool) -> Option<&'static [&'static str]> {
    ...
    if let Some(value) = value.strip_prefix("t-to-") {
        return Some(mask_stop_properties("top", "to", value, infer_named_colors));
    }
    ...
    if let Some(value) = value.strip_prefix("x-from-") {
        return Some(mask_axis_stop_properties(
            "x",
            "from",
            value,
            infer_named_colors,
    ...
    match value {
        ...
        value if is_color_like_value(value, infer_named_colors) => Some(&["mask-image"][..]),
        _ => None,
    }
}

fn mask_radial_properties(value: &str, infer_named_colors: bool) -> &'static [&'static str] {
    if let Some(value) = value.strip_prefix("from-") {
        return mask_stop_properties("radial", "from", value, infer_named_colors);
    }

    if let Some(value) = value.strip_prefix("to-") {
        return mask_stop_properties("radial", "to", value, infer_named_colors);
    }
...
fn mask_stop_properties(
    side: &str,
    stop: &str,
    value: &str,
    infer_named_colors: bool,
) -> &'static [&'static str] {
    match (side, stop, is_color_like_value(value, infer_named_colors)) {
...
fn mask_axis_stop_properties(
    axis: &str,
    stop: &str,
    value: &str,
    infer_named_colors: bool,
) -> &'static [&'static str] {
    match (axis, stop, is_color_like_value(value, infer_named_colors)) {
        ("x", "from", true) => &[
            "mask-image",
...
enum ParenthesizedValueKind {
    Color,
    Length,
}

impl ParenthesizedValueKind {
...
fn is_ring_color_value(value: &str, infer_named_colors: bool) -> bool {
    match ParenthesizedValueKind::parse(value) {
        Some(ParenthesizedValueKind::Color) => true,
        Some(ParenthesizedValueKind::Length) => false,
        None => !value.starts_with('(') && is_color_like_value(value, infer_named_colors),
    }
}

/// Known colors, `current`, and color-kind parenthesized custom properties — no theme-key denylist
///
/// Shadow-family prefixes use this so named values stay ambiguous with custom sizes. Typed
/// non-color parentheses such as `(length:--stop)` are excluded so they can reach the
/// width/position arms that Tailwind assigns them to
fn is_known_color_like_value(value: &str) -> bool {
    match ParenthesizedValueKind::parse(value) {
        Some(ParenthesizedValueKind::Color) => true,
        Some(ParenthesizedValueKind::Length) => false,
        None => !value.starts_with('(') && (is_color_value(value) || value == "current"),
    }
}

fn is_parenthesized_length(value: &str) -> bool {
    ParenthesizedValueKind::parse(value) == Some(ParenthesizedValueKind::Length)
}

/// Color-capable values including theme-key denylist names such as `muted` or `card`
fn is_color_like_value(value: &str, infer_named_colors: bool) -> bool {
    is_known_color_like_value(value) || infer_named_colors && is_named_color_value(value)
}
...
mod tests {
    ...
    fn test_named_design_system_colors() {
        ...
        let cases: &[(&str, &[&str])] = &[
            ...
            ("border-r-muted", &["border-right-color"]),
            ("border-l-muted", &["border-left-color"]),
            ("border-x-input", &["border-inline-color"]),
            ("border-y-input", &["border-block-color"]),
            ...
            ("divide-y-border", &["--tw-divide-color-sort"]),
    ...
    fn test_named_color_inference_can_be_disabled() {
        ...
        for utility in [
            ...
            assert_eq!(
                map.get_properties_with_named_color_inference(utility, false),
                None,
                "{utility}"
            );
        }

        let known_cases: &[(&str, &[&str])] = &[
            ("text-sm", &["font-size"]),
            ("bg-red-500", &["background-color"]),
            ("bg-[#fff]", &["background-color"]),
            ("text-(--color)", &["color"]),
            ("text-(length:--font-size)", &["font-size"]),
        ];
        for &(utility, properties) in known_cases {
            assert_eq!(
                map.get_properties_with_named_color_inference(utility, false),
                Some(properties),
                "{utility}"
    ...
    fn test_named_color_fallback_exclusions() {
        ...
        let cases: &[(&str, &[&str])] = &[
            ...
            ("border-x-2", &["border-inline-width"]),
    ...
    fn test_is_named_color_value() {
        ...
        assert!(is_named_color_value("Primary"));
        ...
        assert!(!is_named_color_value("muted_foreground"));
        assert!(!is_named_color_value("müted"));
        ...
        for keyword in [
            ...
            assert!(
                !is_named_color_value(keyword),
                "{keyword} should be excluded"
            );
        }
    }

    #[test]
    fn test_typed_length_parens_resolve_to_length_properties() {
        let map = UtilityMap::new();

        // Tailwind assigns typed length parentheses to width/position/thickness slots
        assert_eq!(
            map.get_properties("from-(length:--stop)"),
            Some(&["--tw-gradient-from-position"][..])
        );
        assert_eq!(
            map.get_properties("via-(length:--stop)"),
            Some(&["--tw-gradient-via-position"][..])
        );
        assert_eq!(
            map.get_properties("to-(length:--stop)"),
            Some(&["--tw-gradient-to-position"][..])
        );
        assert_eq!(
            map.get_properties("border-(length:--w)"),
            Some(&["border-width"][..])
        );
        assert_eq!(
            map.get_properties("outline-(length:--w)"),
            Some(&["outline-width"][..])
        );
        assert_eq!(
            map.get_properties("decoration-(length:--t)"),
            Some(&["text-decoration-thickness"][..])
        );
        assert_eq!(
            map.get_properties("text-(length:--fs)"),
            Some(&["font-size"][..])
        );
        assert_eq!(
            map.get_properties("stroke-(length:--w)"),
            Some(&["stroke-width"][..])
        );

        // bare custom properties keep resolving as colors
        assert_eq!(
            map.get_properties("from-(--stop)"),
            Some(&["--tw-gradient-from"][..])
        );
        assert_eq!(
            map.get_properties("outline-(--c)"),
            Some(&["outline-color"][..])
        );
        assert_eq!(map.get_properties("text-(--c)"), Some(&["color"][..]));

        // malformed parentheses are not colors
        assert_eq!(map.get_properties("outline-(x)"), None);
    }

    #[test]
    fn test_is_color_like_value_includes_named() {
        assert!(is_color_like_value("red-500", true));
        assert!(is_color_like_value("current", true));
        assert!(is_color_like_value("(--tw-color)", true));
        assert!(is_color_like_value("muted", true));
        assert!(is_color_like_value("muted-foreground", true));

        assert!(is_known_color_like_value("red-500"));
        assert!(is_known_color_like_value("current"));
        assert!(is_known_color_like_value("(--tw-color)"));
        assert!(!is_known_color_like_value("muted"));
        assert!(!is_known_color_like_value("muted-foreground"));
    }
