...
fn bench_custom_sorter(c: &mut Criterion) {
    ...
    let mut custom_map = AHashMap::new();
    ...
    for size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                let class_list = PlainClassList::parse(black_box(&classes_str)).unwrap();
                let sorted = app.sort_class_list(class_list);
                black_box(sorted);
            });
        });
    }
...
fn bench_realistic_component(c: &mut Criterion) {
    ...
    let class_refs: Vec<&str> = component_classes.iter().map(|s| &**s).collect();
